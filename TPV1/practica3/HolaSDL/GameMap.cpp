...
GameMap::GameMap(int fils, int cols, SDLApplication* appPtr) :
	GameObject(Point2D(0, 0), winHeight, winWidth - infoBarWidth, appPtr) {
...
GameMap::GameMap(ifstream& loadFile, SDLApplication* appPtr) { //currentPos se asigna a (0, 0) por la constructora por defecto
	...
	for (int i = 0; i < cols; i++) { //por cada fila
		for (int j = 0; j < fils; j++) { //por cada columna
			if (cells[i][j] != MapCell::Empty) {
				...
				Texture* texture = app->getTexture((TextureName)((int)cells[i][j] - 1));
