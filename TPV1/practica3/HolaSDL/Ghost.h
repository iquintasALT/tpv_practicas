...
public:
	...
	Ghost(Point2D const point, SDLApplication* const appPtr, Point2D const framePos, Point2D const size, PlayState* const playStatePtr)
		: GameCharacter(point, appPtr, framePos, size, playStatePtr) {};
	Ghost(ifstream& loadFile, SDLApplication* const appPtr, PlayState* const playStatePtr);
	virtual ~Ghost() {};
