...
void Texture::load(string filename, uint nRows, uint nCols) {
	...
	if (tempSurface == nullptr) throw "ERROR: loading surface file " + filename + " not found";
	freeSpace();
	texture = SDL_CreateTextureFromSurface(renderer, tempSurface);
	if (texture == nullptr) throw "ERROR: loading texture file " + filename + " not found";
	numRows = nRows;
	numCols = nCols;
	w = tempSurface->w;
	h = tempSurface->h;
